...
- [x] 2) 바그다드를 거쳐 온 점성술 — 2026-09-14 저장소에 올림. **배포 대기** —
      wrangler 토큰이 `kkul-voca` 계정으로 바뀌어 있어 `npx wrangler deploy`가
      인증 오류(code 10000)로 막혔다. byeolsaem은 계정 `55fb977e…`에 있다.
      그 계정으로 `npx wrangler login` 다시 한 뒤 루트에서 배포하면 그대로 나간다.
